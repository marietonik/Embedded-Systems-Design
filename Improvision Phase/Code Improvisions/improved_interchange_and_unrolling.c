...
void read()
{
  ...
  for(j=0;j<M;j++)
  {
    for(i=0;i<N;i=i+4)
    {
      ...
      current_y[i+1][j]=fgetc(frame_c);
      current_y[i+2][j]=fgetc(frame_c);
      current_y[i+3][j]=fgetc(frame_c);
    }
  }
  for(j=0;j<M;j++)
  {
    for(i=0;i<N;i=i+4)
    {
      ...
      current_u[i+1][j]=fgetc(frame_c);
      current_u[i+2][j]=fgetc(frame_c);
      current_u[i+3][j]=fgetc(frame_c);
    }
  }
  for(j=0;j<M;j++)
  {
    for(i=0;i<N;i=i+4)
    {
      ...
      current_v[i+1][j]=fgetc(frame_c);
      current_v[i+2][j]=fgetc(frame_c);
      current_v[i+3][j]=fgetc(frame_c);
    }
  }
...
void write()
{
  ...
  for(j=0;j<M;j++)
  {
    for(i=0;i<N;i=i+4)
    {
      ...
      fputc(current_y_new[i+1][j],frame_yuv);
      fputc(current_y_new[i+2][j],frame_yuv);
      fputc(current_y_new[i+3][j],frame_yuv);
    }
  }
  ...
  for(j=0;j<M;j++)
  {
    for(i=0;i<N;i=i+4)
    {
      ...
      fputc(current_u[i+1][j],frame_yuv);
      fputc(current_u[i+2][j],frame_yuv);
      fputc(current_u[i+3][j],frame_yuv);
    }
  }
  ...
  for(j=0;j<M;j++)
  {
    for(i=0;i<N;i=i+4)
    {
      ...
      fputc(current_v[i+1][j],frame_yuv);
      fputc(current_v[i+2][j],frame_yuv);
      fputc(current_v[i+3][j],frame_yuv);
    }
  }
  fclose(frame_yuv);
}
